...
class _HistoryScreenState extends ConsumerState<HistoryScreen> {
  ...
  void _loadAds() {
    _bannerAd = BannerAd(
      ...
      listener: BannerAdListener(
        ...
        onAdFailedToLoad: (ad, error) {
          ad.dispose();
          AppLogger.debug('BannerAd failed to load: $error');
        },
      ),
    )..load();
    ...
    _nativeAd = NativeAd(
      ...
      listener: NativeAdListener(
        ...
        onAdFailedToLoad: (ad, error) {
          ad.dispose();
          AppLogger.debug('NativeAd failed to load: $error');
        },
      ),
    )..load();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final searchQueryLower = hasSearchQuery ? _searchQuery.toLowerCase() : '';

    final filteredHistory = history.where((scan) {
      final searchMatches = hasSearchQuery
          ? scan.content.toLowerCase().contains(searchQueryLower)
          : true;
      final isToday = scan.timestamp.year == now.year &&
                  ...
                  ListView.separated(
                    ...
                    padding:
                        const EdgeInsets.symmetric(horizontal: 24, vertical: 8)
                            .copyWith(bottom: bottomSafePadding),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final scanOffset = (_isNativeLoaded &&
                              filteredHistory.length >= 3 &&
                              index > 2)
                          ? 1
                          : 0;
                      ...
                      if (scanIndex < 0 ||
                          scanIndex >= filteredHistory.length) {
                      ...
                      return HistoryCard(
                        ...
                        onFavoriteToggle: () =>
                            historyNotifier.toggleFavoriteById(scan.id),
                        onDelete: () => historyNotifier.deleteScanById(scan.id),
                      )
                          .animate()
                          .fade(delay: (index * 30).ms, duration: 300.ms)
                          .slideY(begin: 0.05, end: 0);
                    },
                    itemCount: (_isNativeLoaded && filteredHistory.length >= 3)
            ...
            children: [
              ...
              IconButton(
                ...
                icon: const Icon(Icons.delete_sweep_rounded,
                    color: AppColors.textDimmed, size: 24),
    ...
    return Column(
      children: [
        ...
        SingleChildScrollView(
          ...
          child: Row(
            children: [
              _buildFilterChip('All Scans', _activeFilter == _HistoryFilter.all,
                  () => setState(() => _activeFilter = _HistoryFilter.all)),
              const SizedBox(width: 10),
              _buildFilterChip(
                  'Favorites',
                  _activeFilter == _HistoryFilter.favorites,
                  () =>
                      setState(() => _activeFilter = _HistoryFilter.favorites)),
              const SizedBox(width: 10),
              _buildFilterChip('Recent', _activeFilter == _HistoryFilter.today,
                  () => setState(() => _activeFilter = _HistoryFilter.today)),
            ],
          ),
        ),
      ],
  ...
  Widget _buildEmptyState(
      {required bool isSearching, required _HistoryFilter activeFilter}) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(isSearching ? Icons.search_off_rounded : Icons.history_rounded,
              size: 64, color: AppColors.textDimmed.withValues(alpha: 0.2)),
          const SizedBox(height: 16),
          Text(isSearching ? 'No matches found' : 'Your history is clear',
              style: AppTextStyles.h3.copyWith(color: AppColors.textDimmed)),
        ],
      ),
    );
  }
  ...
  void _showScanResult(BuildContext context, ScanHistory scan) {
    showModalBottomSheet(
      ...
      builder: (context) =>
          ScanResultSheet(content: scan.content, format: scan.format),
    );
  }
  ...
  void _showClearConfirmation(BuildContext context, HistoryNotifier notifier) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(AppStrings.clearHistoryDesc,
            style:
                AppTextStyles.bodyMedium.copyWith(color: AppColors.textDimmed)),
        actions: [
          TextButton(
